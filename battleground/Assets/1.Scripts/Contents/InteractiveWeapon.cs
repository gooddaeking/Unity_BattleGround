...
/// <summary>
/// 충돌체를 생성해 루팅 가능하도록 한다.
/// 루팅하면 충돌체는 제거.
/// 무기를 다시 버리면 충돌체를 다시 붙여준다.
/// 관련해서 UI도 컨트롤 할 수 있어야 한다.
/// ShootBehaviour에 루팅한 무기를 넣어준다.
/// </summary>
public class InteractiveWeapon : MonoBehaviour
{
    ...
    public int currentMagCapacity, totalBullets;        // 현재 탄창 총알량과, 소지 중인 전체 총알량
    ...
    private void Awake()
    {
        ...
        player = GameObject.FindGameObjectWithTag(TagAndLayer.LayerName.Player);
    ...
    public bool Shoot(bool firstShot = true)
    {
        ...
        {
            SoundManager.Instance.PlayOneShotEffect((int)noBulletSound, muzzleTransform.position, 5.0f);
        }
        return false;
    }
