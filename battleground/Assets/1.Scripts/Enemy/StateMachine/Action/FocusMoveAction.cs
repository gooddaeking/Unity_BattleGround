...
public class FocusMoveAction : Action
{
    ...
    private Vector3 currentDest;        // 현재 이동하려고하는 곳
    private bool aligned;               // 타겟을 바라보고 있는가?
    ...
    public override void Act(StateController controller)
    {
        if(!aligned)
        {
            ...
            controller.nav.speed = 0.0f;
            if(controller.enemyAnimation.angularSpeed == 0.0f)
            {
        ...
        else
        {
            ...
            if(controller.hadClearShot != controller.haveClearShot)
            {
                ...
                // 사격이 가능하다면 현재 이동목표가 엄폐물과 다르더라도 일단 이동하지 않는다
