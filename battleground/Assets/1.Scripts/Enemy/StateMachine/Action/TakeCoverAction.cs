...
public class TakeCoverAction : Action
{
    ...
    public override void OnReadyAction(StateController controller)
    {
        ...
        controller.variables.waitInCoverTime = 0.0f;
    ...
    private void Rotating(StateController controller)
    {
        ...
        if(Quaternion.Angle(controller.transform.rotation, targetRotation) > 5.0f)
        {
            controller.transform.rotation = Quaternion.Slerp(controller.transform.rotation, targetRotation, 10.0f * Time.deltaTime);
        }
    }
