...
/// <summary>
/// 마우스 오른쪽 버튼으로 조준. 다른동작을 대체해서 우선 동작.
/// 마우스 휠 버튼으로 좌우 카메라 변경.
/// 벽 모서리에서 조준할 경우 상체를 기울여주는 기능.
/// </summary>
public class AimBehaviour : GenericBehaviour
{
    public Texture2D crossHair;             // 십자선
    public float aimTurnSmoothing = 0.15f;  // 조준할 때 카메라를 향하는 회전속도
    public Vector3 aimPivotOffset = new Vector3(0.5f, 1.2f, 0.0f);  // 조준시 오프셋
    public Vector3 aimCamOffset = new Vector3(0.0f, 0.4f, -0.7f);   // 조준시 오프셋

    private int aimBool;                    // 애니메이터 파라미터. 조준.
    private bool aim;                       // 조준 중 확인
    private int cornerBool;                 // 애니메이터 관련. 코너.
    private bool peekCorner;                // 플레이어 코너 모서리 확인
    ...
    void Rotating()
    {
        ...
        Quaternion targetRotation = Quaternion.Euler(0.0f, behaviourController.GetCamScript.GetH, 0.0f);
        float minSpeed = Quaternion.Angle(myTransform.rotation, targetRotation) * aimTurnSmoothing;

        if(peekCorner)
        {
            // 조준 중일 때 플레이러 상체만 살짝 기울여 주기 위함
