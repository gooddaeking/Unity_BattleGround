...
/// 발사 키 입력받고 애니메이션 재생, 이펙트 생성, 충돌체크.
/// UI 관련 십자선 표시기능. 발사속도 조정.
/// 캐릭터 상체를 IK를 이용해서 조준시점에 맞춰 회전.
/// 벽이나 출돌체에 총알이 충돌되면 피탄이펙트 생성.
/// 인벤토리 역할. 무기를 소지 확인용.
/// 재장전 무기교체 기능.
/// </summary>
public class ShootBehaviour : GenericBehaviour
{
    ...
    private List<InteractiveWeapon> weapons;    // 소지중인 무기들
                            ...
                            bool placeSparks = true, bool placeBulletHole = true)
    {
        ...
        muzzleFlash.transform.localEulerAngles = Vector3.back * 90.0f;
    ...
    private void ShootWeapon(int weapon, bool firstShot = true)
    {
        ...
        else
        {
            ...
            SoundManager.Instance.PlayOneShotEffect((int)weapons[weapon].shotSound, gunMuzzle.position, 5.0f);
